    ...
    if isinstance(X, pd.DataFrame):
        if _is_nested_dataframe(X):
            if X.shape[1] > 1:
                ...
                # we have a single column containing a series, treat this as a univariate problem
                X = _univariate_nested_df_to_array(X)
        else:
            # we have multiple columns each containing a primitive, treat as univariate series
            X = _univariate_df_to_array(X)

    if len(X.shape) == 2:
        # add a dimension to make it multivariate with one dimension
        X = X.reshape(X.shape[0], X.shape[1],
                      1)  # go from [n][m] to [n][m][d=1]
...
def _multivariate_nested_df_to_array(X):
    # tabularise at time of writing will not keep multivariate dimensions separate, e.g. [n][m][d] becomes [n][m*d]
    ...
    return X.reshape(X.shape[0], X.shape[2],
                     X.shape[1])  # go from [n][d][m] to [n][m][d]
